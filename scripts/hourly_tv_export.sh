...
RUNNER=(wa-business-collector)
if ! command -v wa-business-collector >/dev/null 2>&1; then
  if [[ -n "$PYTHON_BIN" ]]; then
    RUNNER=("$PYTHON_BIN" -m wa_business_collector.cli)
  elif [[ -x "$PROJECT_DIR/.venv/bin/python" ]]; then
    RUNNER=("$PROJECT_DIR/.venv/bin/python" -m wa_business_collector.cli)
  elif command -v python3 >/dev/null 2>&1; then
    RUNNER=(python3 -m wa_business_collector.cli)
  else
    RUNNER=(python -m wa_business_collector.cli)
  fi
fi
