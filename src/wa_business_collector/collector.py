...
from wa_business_collector.chrome_session import ChromeWhatsAppSession
from wa_business_collector.models import (
    ChatRow,
    IndexedDBThread,
    LabelStat,
    NormalizedEvent,
...
from wa_business_collector.parsing import parse_chat_list, parse_labels
...
class WhatsAppBusinessCollector:
    ...
    def _effective_excluded_labels(exclude_labels: list[str] | None) -> list[str]:
        ...
        for label in [*DEFAULT_EXCLUDED_LABELS, *(exclude_labels or [])]:
            clean = WhatsAppBusinessCollector._clean_label_name(label)
    ...
    def collect_dashboard_export(
        self,
        *,
        account_label: str = "WhatsApp Business",
        allow_labels: list[str] | None = None,
        exclude_labels: list[str] | None = None,
        max_messages: int = MAX_MESSAGE_LOOKBACK_HARD_LIMIT,
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "account": {
                "platform": "whatsapp-business-web",
    ...
    def _is_trackable_all_view_row(row: ChatRow) -> bool:
        normalized = WhatsAppBusinessCollector._normalized_chat_identity(row.chat_name)
    ...
    def _normalize_label_slug(label: str) -> str:
        cleaned = WhatsAppBusinessCollector._clean_label_name(label).lower()
    ...
    def _normalized_label_set(labels: list[str] | None) -> set[str]:
        return {
            WhatsAppBusinessCollector._normalize_label_slug(label)
    ...
    def _event_from_thread(thread: IndexedDBThread) -> NormalizedEvent:
        return NormalizedEvent(
            ...
            importance=WhatsAppBusinessCollector._importance_for_labels(thread.labels),
            status_hint=WhatsAppBusinessCollector._status_for_labels(thread.labels),
    ...
    def _latest_thread_summary(thread: IndexedDBThread) -> tuple[str | None, str, str | None, str]:
        ...
        last_message_at = WhatsAppBusinessCollector._format_timestamp(thread.last_message_timestamp)
        last_direction = WhatsAppBusinessCollector._infer_direction(thread.preview, thread.unread_count)
    ...
    def _importance_for_labels(labels: list[str]) -> str:
        lowered = {WhatsAppBusinessCollector._clean_label_name(label).lower() for label in labels}
