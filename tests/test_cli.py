...
from wa_business_collector.cli import main
...
class StubCollector:
    ...
    def collect_dashboard_export(self, account_label="WhatsApp Business", allow_labels=None, exclude_labels=None, max_messages=15):
        return {
            "source": "whatsapp",
...
def test_cli_ensure_window_prints_window_payload(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "wa_business_collector.cli.ensure_dedicated_whatsapp_window",
...
def test_cli_quit_profile_terminates_dedicated_instance(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("wa_business_collector.cli.terminate_profile_processes", fake_terminate)
...
def test_cli_status_reports_dedicated_profile_readiness(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "wa_business_collector.cli._status_payload",
        lambda **kwargs: {
