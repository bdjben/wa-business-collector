from wa_business_collector.collector import (
    CHAT_LIST_BODY_JS,
    DEFAULT_EXCLUDED_LABELS,
    LABELS_BODY_JS,
    WhatsAppBusinessCollector,
)
from wa_business_collector.models import ChatRow, IndexedDBThread, LabelStat, NormalizedEvent, RecentMessage, Snapshot
...
def test_plan_all_view_rows_uses_initial_top_15_then_adds_newcomers_up_to_hard_cap() -> None:
    collector = WhatsAppBusinessCollector(session=StubSession())
    initial_rows = [
...
def test_collect_chat_list_prefers_structured_dom_rows_for_all_view_order() -> None:
    ...
    collector = WhatsAppBusinessCollector(session=StructuredChatListSession())
...
def test_collect_labeled_threads_includes_all_labeled_chats_except_archive_label_only_or_excluded_label_only() -> None:
    ...
    collector = WhatsAppBusinessCollector(session=MixedLabelSession())
...
def test_excluded_recent_chat_names_only_excludes_archive_label_only_or_excluded_label_only() -> None:
    ...
    collector = WhatsAppBusinessCollector(session=MixedRecentLabelSession())
...
def test_collect_dashboard_export_emits_labeled_plus_recent_default_view_threads_with_dedupe() -> None:
    ...
    payload = collector.collect_dashboard_export(account_label="WhatsApp Business", allow_labels=["Important", "Follow Up"])
...
def test_collect_dashboard_export_opens_chat_and_uses_true_message_ids_when_indexeddb_has_no_text_for_unlabeled_row() -> None:
    ...
    collector = WhatsAppBusinessCollector(session=session)

    payload = collector.collect_dashboard_export(account_label="WhatsApp Business", allow_labels=[])
...
def test_collect_dashboard_export_resolves_unlabeled_threads_via_chat_aliases_and_phone_digits_without_preview_fallback() -> None:
    ...
    collector = WhatsAppBusinessCollector(session=AliasResolvingSession())
...
def test_latest_thread_summary_prefers_latest_recent_message_over_preview_inference() -> None:
    ...
    assert WhatsAppBusinessCollector._latest_thread_summary(thread) == (
