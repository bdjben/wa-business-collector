...
def test_launch_dedicated_chrome_window_opens_background_chrome_instance(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("wa_business_collector.launcher.subprocess.Popen", fake_popen)
...
def test_terminate_profile_processes_waits_until_profile_processes_exit(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("wa_business_collector.launcher.subprocess.run", fake_run)
    monkeypatch.setattr("wa_business_collector.launcher.time.sleep", lambda *_args, **_kwargs: None)
...
def test_ensure_dedicated_whatsapp_window_launches_when_debug_port_is_not_ready(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("wa_business_collector.launcher.load_display_frames", fake_load_display_frames)
    monkeypatch.setattr("wa_business_collector.launcher.ChromeDevToolsBridge", FakeBridge)
    monkeypatch.setattr("wa_business_collector.launcher.terminate_profile_processes", lambda profile_dir: launched.setdefault('terminated_profile', profile_dir))
    monkeypatch.setattr("wa_business_collector.launcher.launch_dedicated_chrome_window", fake_launch_dedicated_chrome_window)
    monkeypatch.setattr("wa_business_collector.launcher.time.sleep", lambda seconds: sleeps.append(seconds))
...
def test_ensure_dedicated_whatsapp_window_reuses_existing_debug_port(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("wa_business_collector.launcher.ChromeDevToolsBridge", FakeBridge)
    monkeypatch.setattr(
        "wa_business_collector.launcher.launch_dedicated_chrome_window",
